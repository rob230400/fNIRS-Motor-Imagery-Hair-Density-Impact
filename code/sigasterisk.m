function [] = sigasterisk(xbar, ybar, xgroup, ygroup, str_show, bars, varargin)
    ...
    %Lines
    plot([P1_x P2_x], [P1_y P2_y], 'HandleVisibility','off','Color','black', LineWidth=2);
    plot([P2_x P3_x], [P2_y P3_y], 'HandleVisibility','off','Color','black', LineWidth=2);
    plot([P3_x P4_x], [P3_y P4_y], 'HandleVisibility','off','Color','black', LineWidth=2);
    
    %Text
    text(mean([P2_x, P3_x])+threshold(4), mean([P2_y, P3_y])+threshold(3) * sign(mean([P2_y, P3_y])) - threshold(3) * (sign(mean([P2_y, P3_y])) == -1), str_show, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', FontSize=50);
